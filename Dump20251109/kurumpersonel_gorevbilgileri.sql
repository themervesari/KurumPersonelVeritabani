...
CREATE TABLE `gorevbilgileri` (
  ...
  `OlusturmaZamani` timestamp NULL DEFAULT NULL,
  `BaslangicTarihi` date DEFAULT NULL,
  `BitisTarihi` date NOT NULL,
  `AyrilisTarihi` date NOT NULL,
  PRIMARY KEY (`GorevID`),
  KEY `GorevKurumID` (`KurumID`),
  CONSTRAINT `GorevKurumID` FOREIGN KEY (`KurumID`) REFERENCES `kurumbilgileri` (`KurumID`) ON DELETE RESTRICT ON UPDATE CASCADE
